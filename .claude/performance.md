...
- **Küçük client JS** — yalnız `site-nav`, `contact-form`, `cta`, `hero` client. Geri kalan Server. Client sınırı az. 2026-09'da buna Vercel Analytics'in enjekte ettiği küçük script eklendi (`layout.tsx`) — sayfa ağırlığına etkisi ihmal edilebilir, ama dış çalışma-zamanı isteği olduğu için Lighthouse ağ sekmesinde görünecek.
- **Form gönderimi dış istek atar** — `contact-form` submit'te `api.web3forms.com`'a cross-origin `POST` yapar (`application/json` olduğu için bir `OPTIONS` preflight ile birlikte). Yalnız kullanıcı gönder'e bastığında olur; ilk yükleme/LCP üzerinde etkisi yoktur. Vercel Analytics'ten sonraki ikinci dış çalışma-zamanı bağımlılığıdır.
