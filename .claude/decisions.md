...
### 26. F-Gaz bölümüne ton CO₂e hesaplayıcı eklendi (2. sütun), şeftali kartı 1. sütuna taşındı
- **Karar:** Anasayfa F-Gaz bölümünün sağ sütunundaki şeftali kart **1. sütunun en altına** alındı; sağ sütunu artık `FgasCalculator` (`fgas-calculator.tsx`, `"use client"`) dolduruyor. Hesaplayıcı anlıktır — gönderim yok, `/iletisim`'e ya da Web3Forms'a bağlanmaz (kullanıcı kararı).
- **Formül:** `(gaz miktarı (kg) × KIP) / 1000 = ton CO₂e`. Bantlar (`src/lib/fgas.ts` → `CONTROL_BANDS`), **alt sınır dahil**: `<5` yasal zorunluluk yok · `5–50` yılda 1 · `≥50` 6 ayda 1 — yani tam 50 ton 6 ay bandına düşer (`bandFor` sağlanan en yüksek eşiği seçer).
- **2026-09 güncellemesi — yönetmelik değişti:** İlk sürümde 4 bant vardı (`≥500` → 3 ayda 1). Yeni yönetmelikte 500 ton eşiği bir şey ifade etmiyor; o bant kaldırıldı, 50 tonun üzeri tek bant. Değişiklik **yalnız `CONTROL_BANDS`'te** yapıldı — hesaplayıcı bantları diziden render ettiği için UI ve canlı bölge kendiliğinden uydu (referans listesi 4 satırdan 3'e indi, kod değişmedi).
- **Veri yeri:** 19 gazın KIP tablosu + bantlar + formül `src/lib/fgas.ts`'te (bileşen yalnızca sunum). Kullanıcı değerleri verdi; **KIP kaynağı yönetmelik eki değil, işletmenin listesi** — ekip güncellerse tek dosyadan değişir.
- **Trap (bulundu, düzeltildi):** Tarayıcı ayraç ayrıştırması. Türkçe yazımda nokta binlik ayracıdır ama `Number("1.500")` JS'te **1,5** döner — 1500 kg'lık bir chiller sessizce 1,5 kg sanılır ve bant yanlış çıkardı. Sessiz hata olduğu için tehlikeliydi. Çözüm `parseChargeKg`: virgül varsa noktalar binlik olarak atılır (`1.500,5` → 1500.5); virgül yoksa nokta **yalnızca tam üçlü gruplama kuralına** (`^\d{1,3}(\.\d{3})+$`) uyuyorsa binlik sayılır, aksi halde ondalıktır (`12.5` → 12,5). Ayrıştırılamayan dolu girdi sessizce yutulmaz, alan altında uyarı + `aria-invalid` gösterir.
- **Erişilebilirlik:** Sonuç canlı bölgeye **konmadı** — tonaj her tuş vuruşunda değiştiği için ekran okuyucu okunamaz hale gelirdi. Bunun yerine `role="status"` yalnızca **bandı** duyurur (`Kaçak kontrol sıklığı: …`); metin değişmediğinde tarayıcı zaten duyuru tetiklemez, yani duyuru yalnızca 5/50 eşikleri geçilirken çıkar. Girdi yanındaki formül satırı (`12,5 kg × GWP 2.088 ÷ 1000`) ayrıştırmanın şeffaf olmasını sağlar.
- **Ölü kod:** `fgas.tsx` içindeki `RadialGauge` (`%82 dolum oranı`) tanımlı ama hiç render edilmiyordu — karar #17'deki koyu kart tasarımından kalma artık; silindi. `Snowflake`/`Check` importları hâlâ kullanımda.
- **Tasarım:** Hesaplayıcı `bg-mist-gray rounded-[24px]` — **şeftali değil** (şeftali tekilliği kuralı: sayfada 1 yüzey, o da taşınan kart). Shadow yok. Gölge yalnız yüzen artefaktların hakkı olduğu için sonuç bloğu düz `bg-paper-white` ile ayrıştırıldı.
- **Alternatif:** Hesaplayıcıyı form olarak müşteri talebine bağlamak (red — kullanıcı anlık istedi, dönüşüm `Cta` bölümünde kalıyor). Şeftali kartı tamamen kaldırmak (red — "yetkili servis" güvenini taşıyor).
- **Doğrulama:** Bant sınırları R-1234yf (KIP 4 → 1 ton = 250 kg) ile tam sayı çıkarılarak sınandı: 1249/1250/12500 kg → sırasıyla zorunluluk yok / yılda 1 / 6 ayda 1; çok büyük değer (125000 kg = 500 ton) da **6 ayda 1** verir — 500 bandının kalktığının kanıtı. Ayraç vakaları (`12,5` · `12.5` · `1.500`) ve geçersiz girdi uyarısı doğrulandı. Taşma 1440px ve iPhone 12'de `0`. `tsc`, `lint`, `build` temiz.
- **🟡 Açık kalan — hukuki, yayın öncesi gözden geçirilmeli:** (1) Hesaplayıcı çıktısı "yasal zorunluluk" beyanı içeriyor; **sızıntı tespit sistemi bulunan cihazlarda yönetmelik süreleri iki katına çıkarır** — bu kural forma girmedi, sonuç yalnız tonaja göre. (2) R-22 bir HCFC'dir ve Montreal Protokolü kapsamındadır, F-Gaz yönetmeliği değil — listede kullanıcı tercihiyle duruyor. (3) `fgas.tsx`'teki şeftali kart hâlâ "Bakanlık **yetki belgemizle**" diyor, ama karar #24'e göre firmanın F-Gaz yetki belgesi **yok** — bölüm artık daha otoriter göründüğü için bu beyan daha görünür hale geldi. Üçü de kullanıcıya bildirildi, karar bekliyor.
