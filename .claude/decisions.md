...
- **Formül:** `(gaz miktarı (kg) × KIP) / 1000 = ton CO₂e`. Bantlar (`src/lib/fgas.ts` → `CONTROL_BANDS`), **alt sınır dahil**: `<5` yasal zorunluluk yok · `5–50` yılda 1 · `50–500` 6 ayda 1 · `≥500` 3 ayda 1. Eşik anlamı: tam 50 ton → 6 ay bandı, tam 500 ton → 3 ay bandı (`bandFor` sağlanan en yüksek eşiği seçer).
...
- **Erişilebilirlik:** Sonuç canlı bölgeye **konmadı** — tonaj her tuş vuruşunda değiştiği için ekran okuyucu okunamaz hale gelirdi. Bunun yerine `role="status"` yalnızca **bandı** duyurur (`Kaçak kontrol sıklığı: …`); metin değişmediğinde tarayıcı zaten duyuru tetiklemez, yani duyuru yalnızca 5/50/500 eşikleri geçilirken çıkar. Girdi yanındaki formül satırı (`12,5 kg × GWP 2.088 ÷ 1000`) ayrıştırmanın şeffaf olmasını sağlar.
...
- **Doğrulama:** Bant sınırları R-1234yf (KIP 4 → 1 ton = 250 kg) ile tam sayı çıkarılarak sınandı: 1249/1250/12500/125000 kg → sırasıyla zorunluluk yok / yılda 1 / 6 ayda 1 / 3 ayda 1. Ayraç vakaları (`12,5` · `12.5` · `1.500`) ve geçersiz girdi uyarısı doğrulandı. Taşma 1440px ve iPhone 12'de `0`. `tsc`, `lint`, `build` temiz.
